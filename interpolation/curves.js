export default class Curves {
  ...
  static bezier(p, time, points, drawPoints) {
    if (points.length === 1) {
      const point = {
        x: points[0].x,
        y: points[0].y,
      };
      const c = p.lerpColor(p.color(230, 32, 32), p.color(230, 214, 32), time);
      p.push();
      p.noStroke();
      p.fill(c);
      ...
      this.bezier(p, time, ps, drawPoints);
    }
  }

  static catmullRom(p, time, points, drawPoints, alpha) {
    Curves.catmullRom_1(p, time, points, drawPoints, alpha, 0);
  }

  static catmullRom_1(p, time, points, drawPoints, alpha, index) {
    if (points.length - index < 4) return;
    const parts = points.length - 3;

    if (time > (1 / parts) * (index + 1)) {
      this.catmullRom_1(p, time, points, drawPoints, alpha, index + 1);
      return;
    }
    ...
    let blendColor = p.lerpColor(
      p.color(230 - index * 10, 32, 32),
      p.color(230, 214 - index * 10, 32),
      time
    );
